...
&\quad\to \Tms\ \Gamma\ X[\Msub{\mta{out}\ (\mta{dispInIn}\ i)\ X}]) \to \Ty\ (\Gamma \rhd P \rhd X) \to \Tms\ \Gamma\ (P \rhd X) \\
&\mta{dispOut}\ \{\Pi\ A\ B\}\ (a, t)\ u\ M := \mta{dispOut}\ \{B[\Msub a]\}\ t\ u\ M \\
&\mta{dispOut}\ \{\Pi\iota\ p\ B\}\ (y, m, t)\ u\ M := \mta{dispOut}\ \{B\}\ t\ u\ M \\
&\mta{dispOut}\ \{\iota\ p\}\ x\ M := (p, x) \,. \\[1em]
&\mta{dispOp} : \mta{Var}\ T \to ((X, x) : \mta{Alg}\ T) \to \mta{Ty}\ (\Gamma \rhd P \rhd X) \to \Ty\ \Gamma \\
&\mta{dispOp}\ v\ (X, x)\ M := \Pi\ (a : \mta{dispIn}\ (T\ v)\ (X, x)\ M)\ M[\Msub{\mta{dispOut}\ a\ (x\ (i, \mta{dispInIn}\ a))\ M}]\, . \\[1em]
...
\subsubsection{Internal sections}
...
&\mta{cohOpRet}\ v\ i\ m\ f := \mta{Id}\ f[\Msub{\mta{out}\ (\mta{dispInIn}\ i), \mta{dispInIn}\ i}]\ (m\ (v, i)) \\[1em]
...
&\mta{coh}\ \{T = \bullet\}\ (X, x)\ (M, m) := \bullet \\
...
% \end{figure}
  ...
  % &\llbracket \Pi_{\mta{int}}\ p\ B \rrbracket\ X &&:= \Pi\ (X [\langle p \rangle])\ (\llbracket B \rrbracket X)^+ \\
  % &\llbracket \mta{ret}\ p \rrbracket\ X &&:= X [\langle p \rangle]
  % \end{alignat*}
  % \end{minipage}
