...
in a two-level type theory, where signatures are now types in the meta-fragment.
Then we could build up various ways to generate representations, for example
through algebraic ornaments, without needing to laboriously prove induction
principles by hand.
...
We would like to thank the anonymous reviewers for their feedback that significantly improved the quality of this paper.
Additionally, we would like to thank Guillaume Allais for helpful comments on an earlier version of this paper, as well as
Nathan Corbyn, Ellis Kesterton, and Matthew Pickering for interesting discussions on algebraic theories, ergonomics of
inductive types, and pattern synonyms.
