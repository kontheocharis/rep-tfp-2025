...
\begin{itemize}
  \item A dependent type system with inductive families $\lambdaind$
    and its extension by representations $\lambdarep$, along with a translation
    from $\lambdaind$ that erases all represented data types from the program.
  \item A formulation of common optimisations such as the `Nat-hack', and
      ...
      reindexing, using representations.
    \item An prototype implementation of this system in \textsc{Superfluid}, a
    programming language with inductive types and dependent pattern matching.
\end{itemize}
