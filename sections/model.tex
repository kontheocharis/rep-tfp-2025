...
% \subsection{Filtering compilation}

% \begin{definition}
%   A \emph{filtered} compilation functor $\fun C$ between 2-categories $\cat S$
%   and $\cat L$ is a functor
%   %
%   $$
%     \fun C : \cat S \funArrow \cat L + \mathbf{F}
%   $$
%   %
%   separating the objects of $\cat S$ into those that are representable in $\cat
%     L$ and those that are not, by means of a discrete category $\mathbf{F}$ of
%   ``error objects".
% \end{definition}
...
% constructs in the high-level language might not have a direct representation
% in the low-level language. For example, in a dependently typed language, a
