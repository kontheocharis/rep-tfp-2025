...
A representation definition is part of the global context of a program, which
can access top-level symbols. The requirements for representing an inductive type family $\datalab{D}$,
...
\begin{itemize}
  \item A telescope of parameters $\Delta$, which cannot vary between constructors.
...
This will effectively erase the \datalab{Nat} type from the compiled program, replacing
all occurrences with the $\primlab{UBig}$ type and its primitives.

If we have the additional equality
\[
