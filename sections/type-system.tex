...
\end{align*}
as well as the telescopic internal equivalents $\mta{dispAlg} : \Tel\ (\Gamma \rhd \mta{alg}\ T)$. %
% \begin{align*}
%     &\mta{dispAlg} : \Tel\ (\Gamma \rhd \mta{alg}\ T) \\
%     &\mta{dispAlg} := a.\ \bullet \rhd (M : \Pi\ P\ (\Pi\ X\ \univ)) \rhd \mta{dispAlgebra}\ \emb{a}\ (\mta{El}\ M@@) \,.
% \end{align*}
Internal displayed algebras come with realisation functions $\emb{\_}$ too.
...
\begin{align*}
    ...
    &\emb{\_}_1\ : (i : \mta{Tms}\ \Gamma\ (\Pi\ \mta{dispAlg}[\Msub X]\ \mta{section}[\Msub X])) \to (m : \mta{DispAlg}\ X) \to \mta{IntCoh}\ \emb{i}_0\ m \,.
\end{align*}
