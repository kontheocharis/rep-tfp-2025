...
of Martin-L\"of type theory \cite{Martin-Lof1984-pz}, extended to telescopes:
...
We will use square brackets for substitution: if $\Gamma, A \vdash B$
and $\Gamma \vdash a : A$ then $\Gamma \vdash B[a]$.
...
\[\Gamma \vdash \sigma[\alpha.\textit{zero}] = \textit{zero}_Y : Y[\alpha.\textit{zero}] \qquad \Gamma \vdash \sigma[\alpha.\textit{succ}\ x] = \textit{succ}_Y\ x\ \sigma[x] : Y[\alpha.\textit{succ}\ x].\]
In the latter case, we have a telescope of propositional equality witnesses
\begin{align*}
\Gamma \vdash (&\n{zero}_{\text{coh}} :  \sigma[\alpha.\textit{zero}] \equiv \textit{zero}_Y,\ \n{succ}_{\text{coh}} : \sigma[\alpha.\textit{succ}\ x] \equiv \textit{succ}_Y\ x\ \sigma[x]) \tel.
\end{align*}
...
$\Gamma \vdash \beta\COH\ \sigma \tel$ & Coherence conditions for a section $\Gamma, \Delta, X \vdash \sigma : Y$ of a displayed algebra $\Gamma \vdash \beta :: \alpha\D\ Y$.
\end{definitions}
