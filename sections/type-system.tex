...
when de-Brujin indices are implied. We use $@$ for `partial' object applications
as the inverse of $\lambda$: applying $f$ to $x$ is $(f @)[\Msub{x}]$.
...
$\mta{alg}\ T := \bullet \rhd (X : \Pi\ P\ \univ) \rhd \mta{algebra}\ T\ (\mta{El}\ X@)$ where
\begin{align*}
...
\end{align*}
and we can use the realisation function for the internal equivalent of
$\mta{DispAlg}$, $\mta{dispAlg} : \Tel\ (\Gamma \rhd \mta{alg}\ T)$. %
% \begin{align*}
...
As a result, the realisation functions for sections
...
produce only internal coherence proofs.
...
with realisations
\begin{align*}
    &\emb{\_}_0\ : \mta{Tms}\ \Gamma\ (\Pi\ \mta{dispAlg}[\Msub X]\ \mta{section}[\Msub X]) \to (m : \mta{DispAlg}\ X) \to \mta{Sec}\ m \\
    &\emb{\_}_1\ : (i : \mta{Tms}\ \Gamma\ (\Pi\ \mta{dispAlg}[\Msub X]\ \mta{section}[\Msub X])) \to (m : \mta{DispAlg}\ \emb{X}) \to \mta{IntCoh}\ \emb{i}_0\ m \,.
\end{align*}
...
We reuse the \mta{Sig} type defined in the previous section. This allows us
...
$\Gamma$. It is evidently a decidable relation.
