...
with inductive families $\lambdadata$ with data representations. The base
...
to convert between inductive types and their representations. Finally, in \cref{sub:translation} we define a
...
\begin{figure}[H]
  ...
  \and
  \inferrule[Tel-Extend]{\Gamma \vdash A \type \\ \Gamma, A \vdash \Delta \tel}{\Gamma \vdash (A, \Delta) \tel}
  \\
  \inferrule[Spine-Empty]{ }{\Gamma \vdash (\,) :: \bullet} \and
  \inferrule[Spine-Extend]{\Gamma \vdash a : A \\ \Gamma \vdash \delta : \Delta[a]}{\Gamma \vdash (a, \delta) :: (A, \Delta)}
  \end{mathpar}
...
\begin{figure}[H]
  ...
  \and
  \inferrule[Sig-Extend]{\Gamma \vdash \Delta \tel \\ \Gamma \vdash O \op \Delta \\ \Gamma \vdash S \sig \Delta}{\Gamma \vdash (O \lhd S) \sig \Delta}
  \\
...
$\Gamma \vdash (\iota\ (\,) \lhd \iota\ (\,) \to\internal \iota\ (\,) \lhd \epsilon) \sig \bullet$.
We can add labels to aid readability, omit index spines if they are empty, and omit the final $\epsilon$ from signatures:
\[
\Gamma \vdash (\textit{zero} : \iota \lhd \textit{succ} : \iota\ \to\internal \iota) \sig \bullet \,.
\]
\end{example}
...
\inferrule[$\axiom{Repr-Id}_1$]{\Gamma \vdash a : \MRepr\ A}
{\Gamma \vdash \Mrepr\ (\Munrepr\ a) = a : \MRepr\ A} \and
\inferrule[$\axiom{Repr-Id}_2$]{\Gamma \vdash a : A}
{\Gamma \vdash \Munrepr\ (\Mrepr\ a) = a : A} \and
\inferrule[Repr-Data]{\Gamma \vdash S \sig \Delta \\ \Gamma \vdash \gamma :: S\INDA \\ \Gamma \vdash \delta :: \Delta}
...
these types are `the same'. In other words, $\MRepr\ A \simeq A$ but not $\MRepr\ A
= A$. All the contextual machinery of general modal type systems
\cite{Gratzer2020-kf} is not necessary here because this modality is fibred over
contexts so it presents as just a type former.

One might hope for additional computation rules. For example, the representation of a constructor
...
definitional equality rules would imply that it is (due to $\axiom{Repr-Id}_2$).
...
without breaking conversion. These are given for $\univ$, $\Pi$, $\Sigma$, unit and
identity types in the Agda formalisation.
...
\MRepr : \univ \to \univ_C
$$
targets a universe of `concrete' types $\univ_C < \univ$ closed under all standard type
formers, but which does not contain any $\Mdata$ types. We can then require that
all inductive algebras $\gamma$ used in the rule \axiom{Data-Form} must have a
concrete carrier $X : \Delta \to \univ_C$. This does not limit
...
$\R$ replaces data types with their underlying inductive algebras. We use the
notation $\Ty$ and $\Tm$ for types and terms respectively, with a subscript
...
All the mappings above are clearly structurally recursive.
The translation is extended to contexts, substitutions, telescopes and spines
pointwise, and all the rest of the syntax is preserved: $\R((a : A) \to B[a]) = (a : \R A) \to
(\R B)[a]$ etc. All $\Mdata$ types are translated to the carriers of their inductive
...
The isomorphism of $\Mrepr/\Munrepr$, as well as the rule \axiom{Data-Repr} are
...
We have formalised this transformation in Agda.
