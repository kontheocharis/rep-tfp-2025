...
Consider extending the languages $\lambdarep$ and $\lambdarep$ with subset $\Sigma$-types
\[
...
|x|$. Then if we have an inductive family $G : \mta{data}\ (\bullet \rhd I)\ T_G
...
&\mta{forget}_i = g.\ \pi_1\ (\mta{repr}\ g) \\[1em]
&\mta{remember} : \Tm\ \Gamma\ (\Pi\ (x : \mta{D}\ F)\ (\mta{D}\ G[\mta{app}\ f\ x])) \\
&\mta{remember} = x.\ (\mta{unrepr}\ x, \mta{refl})
\end{align*}
