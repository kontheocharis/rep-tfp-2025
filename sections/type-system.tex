...
The actual type of natural numbers $\Gamma \vdash \mathbb{N} \type$ is just the
...
\cref{fig:translation} from \lambdadata to extensional \langmltt, meant to be
...
decidable for $\lambdadata$\footnote{Not formalised in this paper.} and we only
need to apply this transformation after type checking, on fully-typed terms. The
translation is defined over the syntax of $\lambdadata$ \cite{Boulier2017-cm}
such that definitional equality is preserved, which is shown in
...
All the mappings above are clearly structurally recursive. The translation is
...
$\Mrepr/\Munrepr$, as well as the rule \axiom{Data-Repr} are preserved by
...
with W-types or something similar. One especially convenient choice is to extend
...
Consider extending our languages with usage-aware `subset' dependent pairs
\[
