...
\begin{figure}[h]
  \begin{mathpar}
    ...
    \inferrule[Ctor-Item]
    {
      ...
      \forall j \in J .\ \Sigma \mid \delta : \Delta, \Pi \vdash r_j : \Xi[\delta] \\
      \Sigma \mid \Delta, \Pi \vdash \xi : \Xi
    }
    {\Sigma \vdash \Sctor{C}{\Pi\;\vec{r} }{D}{\Delta\;\xi}} \and
    \inferrule[Closed-Item]
    {
      \Sdata{D}{\Delta}{\Xi} \in \Sigma \\
      \forall i \in I. \ \Sctorvar{\lab{C}_i}{\Pi_i\;\vec{r}_i}{D}{\Delta\;\xi_i} \in \Sigma \\
      \Sclosed{\lab{D}, \wildp} \notin \Sigma
    }
...
\begin{figure}[H]
  \begin{mathpar}
    ...
    {\Sigma \mid \Gamma \vdash \lab{D} : (\delta : \Delta) \to \Xi [\delta] \to \univ} \and
    \inferrule[Data-Intro]
    {
      \Sdata{D}{\Delta}{\Xi} \in \Sigma     \\
      \Sctor{C}{\Pi\;\vec{r}}{D}{\Delta\;\xi} \in \Sigma
    }
    {\Sigma \mid \Gamma \vdash \lab{C} : (\delta : \Delta) \to (\pi : \Pi [\delta]) \to \{ \lab{D} \; \delta \; (r_j [\delta, \pi]) \}_j \to \lab{D} \; \delta \; (\xi [\delta, \pi]) } \and
    \inferrule[Data-Elim]
    {
      \Sdata{D}{\Delta}{\Xi} \in \Sigma     \\
      \forall i \in I.\ \Sctorvar{\lab{C}_i}{\Delta_{\lab{C}_i}}{D}{\xi_i} \in \Sigma     \\
      \Sclosed{\lab D}{\vec{\lab{C}}} \in \Sigma
    }
    {\Sigma \mid \Gamma \vdash \lab{elim_D} : (\delta : \Delta) \to \mta{Elim}((\xi : \Xi[\delta],x : \lab{D} \; \delta \; \xi), P.\{ (\pi : \Pi [\delta]) , \{ P \; \delta \; (r_j [\delta,\pi]) \}_j \}_i ) } \and
    \inferrule[Def-Intro]
    {
      \Sdef{f}{M}{m} \in \Sigma     \\
    }
...
In \cref{fig:lambdaind-rules}, the $\mta{Elim}(\Phi, P.\{\Delta_k\}_k)$ construction is defined as
\begin{mathpar}
  \inferrule
  {
    \Sigma \mid \Gamma \vdash \isTel{\Phi} \\
    \forall k \in K .\ {\Sigma \mid \Gamma, P : \Phi \to \univ \vdash \isTel{\Delta_k}}
  }
  {\mta{Elim}(\Phi, P.\{\Delta_k\}_k) = (P : \Phi \to \univ) \to \{ \Delta_k \to P \; \xi_k \}_k \to (\phi : \Phi) \to P \phi}
\end{mathpar}
