...
\begin{figure}[h]
  \begin{mathpar}
    ...
    {\Sigma \vdash \Sctor{C}{\Pi}{D}{\Delta\;\xi}} \and
...
\begin{figure}[H]
  \begin{mathpar}
    ...
    {\Sigma \mid \Gamma \vdash \lab{D} : \Delta \to \Xi \to \univ} \and
    \inferrule[Data-Intro]
    {
      \Sdata{D}{\Delta}{\Xi} \in \Sigma     \\
      \Sctor{C}{\Pi}{D}{\Delta\;\xi} \in \Sigma
    }
    {\Sigma \mid \Gamma \vdash \lab{C} : (\delta : \Delta) \to (\pi : \Pi [\delta]) \to \lab{D} \; \delta \; (\xi [\delta, \pi]) } \and
    \inferrule[Data-Case]
    {
      \Sdata{D}{\Delta}{\Xi} \in \Sigma     \\
      \forall i \in I.\ \Sctorvar{\lab{C}_i}{\Pi_i}{D}{\xi_i} \in \Sigma     \\
      \Sclosed{\lab D}{\vec{\lab{C}}} \in \Sigma
    }
    {\Sigma \mid \Gamma \vdash \lab{case_D} : (\delta : \Delta) \to \mta{Case}((\xi : \Xi[\delta],x : \lab{D} \; \delta \; \xi), \{ (\pi : \Pi_i [\delta]), (\xi_i [\delta], \lab{C}_i \; \delta \; \pi) \}_i ) } \and
    ...
    %   \Sigma \mid \Gamma \vdash P : (\xi : \Xi[\delta]) \to \lab{D} \; \delta\; \xi \to \univ \\
    %   \forall i \in I . \ \Sigma \mid \Gamma \vdash \kappa_i : (\pi : \Pi_i [\delta]) \to P \; \xi_i [\delta, \pi] \; (\lab{C}_i \; \delta \; \pi)  \\
    %   \Sigma \mid \Gamma \vdash \pi : \Pi_j [\delta]
    % }
    % {\Sigma \mid \Gamma \vdash \lab{case_D} \; \delta \; P \; \{\kappa_i\}_i \; (\xi_j [\delta, \pi]) \; (\lab{C}_j \; \delta \; \pi)
    \inferrule[Data-Case-ID$_j$]
    {
      \Sdata{D}{\Delta}{\Xi} \in \Sigma     \\
      \forall i \in I.\ \Sctorvar{\lab{C}_i}{\Pi_i}{D}{\xi_i} \in \Sigma     \\
    ...
    {\Sigma \mid \Gamma \vdash \mta{IsValidCase}(\lab{case_D}\;\delta, P, \{\kappa_i\}_i)} \and
    \inferrule[Def-Intro]
    {
      \Sdef{f}{M}{m} \in \Sigma     \\
    }
...
\begin{mathpar}
  ...
  {\Sigma \mid \Gamma \vdash \mta{ValidCase}(c, P, \{\kappa_k\}_k)}
\end{mathpar}
...
\begin{figure}[h]
  \begin{mathpar}
    ...
    {\Sigma \vdash \Srepr{D}{\Delta\;\Xi}{A}} \and
    \inferrule[Repr-Ctor]
    {
    \Sdata{D}{\Delta}{\Xi} \in \Sigma     \\
    \forall i \in I .\ \Sctorvar{\lab{C}_i}{\Pi_i}{D}{\Delta\;\xi_i} \in \Sigma     \\
    \Srepr{D}{\Delta\;\Xi}{A} \in \Sigma \\
    \forall i < k .\ \Sreprvar{\lab{C}_i}{\Pi_i}{t_i} \in \Sigma     \\
    ...
    {\Sigma \vdash \Sreprvar{\lab{C}_k}{\Pi_k}{t_k}} \and
    \inferrule[Repr-Case]
    {
    \Sdata{D}{\Delta}{\Xi} \in \Sigma     \\
    \Srepr{D}{\Delta\;\Xi}{A} \in \Sigma \\
    \forall i \in I .\ \Sctorvar{\lab{C}_i}{\Pi_i}{D}{\Delta\;\xi_i} \in \Sigma     \\
    \forall i \in I .\ \Sreprvar{\lab{C}_i}{\Pi_i}{t_i} \in \Sigma     \\
    \Sclosed{\lab{D}}{\vec{\lab{C}}} \in \Sigma \\
    {\Sigma \mid \Delta \vdash c : \mta{Case}((\xi : \Xi[\delta],x : A \; \delta \; \xi), \{ (\pi : \Pi_i [\delta]), (\xi_i [\delta], A \; \delta \; \pi) \}_i ) } \\
    \Sigma \mid \Delta \vdash \mta{ValidCase} ( c )
    }
    {\Sigma \vdash \Srepr{case_D}{\Delta}{c}} \and
    \inferrule[Repr-Def]
    {
      \Sdef{f}{M}{m} \in \Sigma     \\
      % \isLabNotIn{R}{\Sigma} \\
...
%         $\inte{or}$, and $\inte{not}$. Caseination for booleans is also provided, in
...
% \cref{fig:lambdaind-rules}. \todo{Here we must define the equality rules for
%   constructors} For brevity, only the rules that relate to items in signatures
...
% A data definition is represented by an object-level type $A$ which is allowed
...
% \todo{This argument should be made more formal} If such a representation for
...
% There is another way to view data representations that might elucidate the
...
% extensionality, so that equality of functions is defined pointwise. \todo{We
%   really need to use the proofs to show some formal correctness argument..}
