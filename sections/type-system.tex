...
We will often use the notation $\delta.y$ to extract a certain index $y$ from a spine $\delta$.
This is used when we define telescopes using named notation. For example, if $\delta :: (\textit{X} : A \to \univ, \textit{y} : (a : A) \to X\ a)$,
then $\delta.X : A \to \univ$ and $\delta.y : (a : A) \to \delta.X\ a$.
...
interpret their structure as types in the type theory. An \emph{algebra} for a signature
...
of the type theory.
An algebra over the signature of natural numbers over $\Gamma \vdash X \type$ is a spine matching
the telescope $(\textit{zero}: X,\ \textit{succ}: X \to X)$.

The \emph{actual} natural numbers $\Gamma \vdash \mathbb{N} \type$
are just an algebra over the signature of natural numbers. In particular, they
are the `best' such algebra; one whose operations do not forget any information.
In the language of category theory, these are the initial algebras in the
category of algebras over the signature of natural numbers. An equivalent
formulation of initial algebras is algebras which support induction, which is
more suitable for our (syntactic) purposes.

Induction over an algebra $\alpha :: (\textit{zero}: X,\ \textit{succ}: X \to X)$
means:
\begin{quote}
For any type family $Y : X \to \univ$,
...
The produced function $\sigma : (x : X) \to Y\ x$ is a \emph{section} of the type family $Y$.
Induction also requires that the section acquires its values from the provided methods.
This means that $\sigma\ \alpha.\textit{zero} = \textit{zero}_Y$ and $\sigma\
(\alpha.\textit{succ}\ x) = \textit{succ}_Y\ x\ (\sigma\ x)$. We call this the \emph{coherence condition}.
...
type family over another algebra. These are \emph{displayed algebras}. In
...
argument $x : X$ as well as an argument $y : Y\ x$. Again, the function arrows
in $S$ are interpreted as function types in the type theory.

Finally, we come to the central definition that classifies the algebras which
support induction:
...
section). Furthermore this function satisfies an appropriate computation rule
when we plug in a constructor (the coherence condition).

\subsection{Defining algebras and friends}

In order to utilise these constructions for our type system, we define explicitly
the following objects:

\begin{itemize}
\item For a signature $\Gamma \vdash S \sig \Delta$, the algebras over a carrier $\Gamma, \Delta \vdash X \type$:
      \[
      \Gamma \vdash S\A\ X \tel
      \]
\item For an algebra $\Gamma \vdash \alpha :: S\A\ X$, the displayed algebras over a carrier family $\Gamma, \Delta, X \vdash Y \type$:
        \[
        \Gamma \vdash \alpha\D\ Y \tel
        \]
\item For a displayed algebra $\Gamma \vdash \beta :: \alpha\D\ Y$, and section $\Gamma \vdash \sigma : (x : X) \to Y\ x$, the coherence conditions:
        \[
        \Gamma \vdash \beta\COH\ \sigma \tel
        \]
\end{itemize}
